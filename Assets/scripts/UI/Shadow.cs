...
public class Shadow : MonoBehaviour
{
  ...
  //  GameInstance gameInstance = new GameInstance();
    ...
    void Update()
    {
        ...
        if (QualitySettings.GetQualityLevel() > 1) GetRectTransform.localScale = new Vector3(f, f, f);
        else GetRectTransform.localScale = new Vector3(0, 0, 0);
        if (model !=null)
        {
            Vector3 rectPosition = new Vector3(model.position.x - offsetX, 0.1f + model.position.y, model.position.z - offsetZ);
            Vector3 rectEuler = new Vector3(90, 0, -model.rotation.eulerAngles.y);
